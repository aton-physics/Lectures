    ...
    def plot(self, pt = 'trajectory'):
        ...
        if hasattr(self,'xv'):
            ...
            if pt == 'phase':
                ...
                #too much information inside xv[:, 0] and xv[:, 1] which contain our theta and omega information, respectively.
                #To do this, I will create a new list generated from xv but only containing points that satisfy this conditional
                #In my case, this conditional is going to be | t - 3*pi*n| < 0.005/2 where n is any integer
                const = 6*3.14159/2 
                #counter = []
                counter = [101787,
103672,
...
197919,
199804]
                '''for it in range(len(self.tarray)):
                    for ct in range(1000):
                        if (self.tarray[it] > 100 and np.abs(self.tarray[it] - const*ct) < self.dt / 2):
                            print(it)
                            counter.append(it)'''
                Index = 0
                newxv = np.zeros((53,2))
                for it in counter:
                    newxv[Index][0] = self.xv[it][0]
                    newxv[Index][1] = self.xv[it][1]
                    Index += 1
                #ax1.plot(newxv[:,0], newxv[:,1], "k",'-', label = 'odeint')
                plt.scatter(newxv[:,0], newxv[:,1])
                plt.xlim(-.1, 1.2)
                plt.ylim(-1.5, 1.5)
